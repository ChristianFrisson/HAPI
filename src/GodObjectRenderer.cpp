...
void GodObjectRenderer::onOnePlaneContact( const PlaneConstraint &c, 
...
void GodObjectRenderer::onTwoPlaneContact( const PlaneConstraint &p0,
...
GodObjectRenderer::renderHapticsOneStep( HAPIHapticsDevice *hd,
  ...
  vector< Bounds::IntersectionInfo > closest_intersections;
  ...
  for( HapticShapeVector::const_iterator i = shapes.begin();
    ...
    if( (*i)->lineIntersect( proxy_pos, input.position, intersection ) ) {
      ...
      if( !has_intersection ) {
        ...
        closest_constraints.push_back( PlaneConstraint( intersection.point, 
                                                        intersection.normal, 
                                                        *i ) );
      } else {
        Vec3 v = intersection.point - proxy_pos;
  ...
  if( nr_constraints == 0 ) {
    ...
    new_proxy_pos = 
      closest_intersection.point + closest_intersection.normal * 1e-1 +
